...
export function BenchmarksDashboard() {
  ...
  const handleAllBenchmarksComplete = useCallback((results: Map<string, BenchmarkResult>) => {
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {sortedModels.map((model) => {
          ...
          return (
            <Card key={model.id} className={model.isAvailable === false ? 'opacity-60' : ''}>
              <CardHeader className="pb-2">
                <div className="flex justify-between items-start">
                  ...
                  <div 
                    className={`w-3 h-3 rounded-full ${model.isAvailable === false ? 'bg-red-500' : 'bg-green-500'}`}
                    title={model.isAvailable === false ? 'Unavailable' : 'Available'}
                  />
                </div>
              </CardHeader>
                ...
                <div className="grid grid-cols-2 gap-y-2 gap-x-4">
                  ...
                  {benchmarkResult && (
                    <div className="flex items-center gap-2 col-span-2">
                      ...
                      <div>
                        <div className="text-sm font-medium">
                          {benchmarkResult.totalTime.toFixed(2)}s for {benchmarkResult.completionTokens} tokens
                        </div>
                        <div className="text-xs text-muted-foreground">
              ...
              <CardFooter>
                ...
                {benchmarkResult && (
                  <div className="ml-auto text-xs text-muted-foreground">
                    Last run: {new Date(benchmarkResult.timestamp).toLocaleTimeString()}
                  </div>
                )}
              </CardFooter>
            </Card>
      ...
      <div className="mt-8 p-4 border rounded-lg bg-muted/50">
        ...
        <p className="mb-2">
          These benchmarks measure model performance using real API calls. The key metrics include:
        </p>
        <ul className="list-disc pl-6 space-y-1 mb-4">
          ...
          <li><strong>Total Time</strong>: Overall time to complete the response</li>
        </ul>
        <p className="text-sm text-muted-foreground">
          Benchmark results are cached for one hour. Results may vary based on network conditions, 
          server load, and other factors. For more precise benchmarking, consider using specialized 
          tools like LLMPerf by Anyscale or Artificial Analysis's LLM Performance Benchmark.
        </p>
      </div>
    </div>
  );
